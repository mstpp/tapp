...
impl TapStorage {
    ...
    pub fn read(file_path: &str) -> Option<Value> {
        let read_from_storage = std::fs::read_to_string(file_path).unwrap_or("{}".to_string());
...
impl Tap {
    ...
    pub fn history() -> String {
        let ts_format = format_description!("[year]-[month]-[day] [hour]:[minute]:[second] UTC");
        ...
        for tap_item in tap_vec.iter() {
            let unix_ts = tap_item["timestamp"].as_i64().unwrap_or_default();
            let mut td = UtcDateTime::from_unix_timestamp(unix_ts)
                .unwrap()
                .format(&ts_format)
                .unwrap();
